...
AppleTCPIPv4Socket::AppleTCPIPv4Socket() {
}
AppleTCPIPv4Socket::AppleTCPIPv4Socket(const TCPIPv4Address& address)
...
void AppleTCPIPv4Socket::initialize() {
...
void AppleTCPIPv4Socket::destroy() {
	ASSERT(fd >= 0);
...
TCPIPv4Connection* AppleTCPIPv4Socket::acceptImpl() {
	sockaddr_in saddr;
	...
	if (accepted >= 0) {
		{
			int value = 1;
			int res = ::setsockopt(accepted, SOL_SOCKET, SO_NOSIGPIPE, &value, sizeof(value));
			ASSERT(res == 0) << "syscall failed " << strerror(errno);
		}
