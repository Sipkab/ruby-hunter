...
AppleUDPIPv4Socket::AppleUDPIPv4Socket(const UDPIPv4Address& address)
		: UDPIPv4SocketBase(address) {
}
AppleUDPIPv4Socket::AppleUDPIPv4Socket(AppleUDPIPv4Socket&& o)
		: UDPIPv4SocketBase(util::move(o)), fd(o.fd) {
	o.fd = -1;
}
AppleUDPIPv4Socket& AppleUDPIPv4Socket::operator =(AppleUDPIPv4Socket&& o) {
...
AppleUDPIPv4Socket::~AppleUDPIPv4Socket() {
	if (fd >= 0) {
		destroy();
	}
}

void AppleUDPIPv4Socket::initialize() {
...
void AppleUDPIPv4Socket::destroy() {
	ASSERT(fd >= 0);
...
bool AppleUDPIPv4Socket::sendPacketImpl(const NetworkAddress& netaddress, const void* data, unsigned int bytecount) {
...
int AppleUDPIPv4Socket::receivePacketImpl(void* data, unsigned int bytecount, NetworkAddress* netaddress) {
	ASSERT((netaddress->getAddressType() & (NetworkAddressType::MASK_IP_VERSION)) == NetworkAddressType::IPV4);
