...
OpenGl30Texture::OpenGl30Texture(OpenGl30Renderer* renderer)
		: renderer { renderer } {
}
OpenGl30Texture::~OpenGl30Texture() {
}

bool OpenGl30Texture::applyTextureData() {
...
bool OpenGl30Texture::reload() {
	return applyTextureData();
}
GLint OpenGl30Texture::bind() {
	...
	if (bindPtr == nullptr) {
		...
		CHECK_GL_ERROR();
		renderer->glBindTexture(GL_TEXTURE_2D, name);
		CHECK_GL_ERROR();
	} else {
		//has binding already
...
void OpenGl30Texture::initAsEmpty(unsigned int width, unsigned int height, ColorFormat format) {
...
void OpenGl30Texture::initWithData(unsigned int width, unsigned int height, ColorFormat format, const void* data) {
	const int internalformat = translateGlInternalFormat(format);
	const int type = translateGlType(format);
...
void OpenGl30Texture::initFailed() {
	renderer->glDeleteTextures(1, &name);
	CHECK_GL_ERROR();
	name = 0;
}
