...
class OpenGl30IndexBuffer final : public render::IndexBuffer {
...
private:
	...
	virtual void allocateDataImpl(const void* data, unsigned int bytecount, BufferType bufferType) override {
		...
		renderer->glBufferData(GL_ELEMENT_ARRAY_BUFFER, bytecount, data, OpenGl30Renderer::convertBufferUsageType(bufferType));
		CHECK_GL_ERROR();
	}
...
protected:
	...
	OpenGl30IndexBuffer(OpenGl30Renderer* renderer)
			: renderer { renderer } {
	}
public:
	~OpenGl30IndexBuffer() {
