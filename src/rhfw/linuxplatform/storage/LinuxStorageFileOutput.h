 ...
 */
/*
 * LinuxStorageFileOutput.h
 *
 *  Created on: 2015 okt. 26
 *      Author: sipka
 */

#ifndef LINUXSTORAGEFILEOUTPUT_H_
#define LINUXSTORAGEFILEOUTPUT_H_
...
class LinuxStorageFileOutput final: public FileOutput {
private:
	FilePath path;
	int fd;
...
public:
	LinuxStorageFileOutput(FilePath path)
	: path {util::move(path)}, fd {-1} {
	}
	LinuxStorageFileOutput(LinuxStorageFileOutput&& o)
	: FileOutput {util::move(o)}, path {util::move(o.path)}, fd {o.fd} {
		o.fd = -1;
	}
	~LinuxStorageFileOutput() {
		close();
	}
...
#endif /* LINUXSTORAGEFILEOUTPUT_H_ */
