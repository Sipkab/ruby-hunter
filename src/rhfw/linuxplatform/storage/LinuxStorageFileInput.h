 ...
 */
/*
 * LinuxStorageFileInput.h
 *
 *  Created on: 2015 okt. 26
 *      Author: sipka
 */

#ifndef LINUXSTORAGEFILEINPUT_H_
#define LINUXSTORAGEFILEINPUT_H_
...
class LinuxStorageFileInput final: public FileInput {
private:
	friend class LinuxStorageFileDescriptor;
...
public:
	LinuxStorageFileInput(FilePath path)
			: path(util::move(path)), fd(-1) {
	}
	LinuxStorageFileInput(LinuxStorageFileInput&& o)
			: FileInput(util::move(o)), path(util::move(o.path)), fd(o.fd) {
		o.fd = -1;
	}
	virtual ~LinuxStorageFileInput() {
		close();
	}
...
#endif /* LINUXSTORAGEFILEINPUT_H_ */
