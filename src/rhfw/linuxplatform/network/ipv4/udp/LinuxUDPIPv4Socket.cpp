...
LinuxUDPIPv4Socket::LinuxUDPIPv4Socket(const UDPIPv4Address& address)
		: UDPIPv4SocketBase(address) {
}
LinuxUDPIPv4Socket::LinuxUDPIPv4Socket(LinuxUDPIPv4Socket&& o)
		: UDPIPv4SocketBase(util::move(o)), fd(o.fd) {
	o.fd = -1;
}
LinuxUDPIPv4Socket& LinuxUDPIPv4Socket::operator =(LinuxUDPIPv4Socket&& o) {
...
LinuxUDPIPv4Socket::~LinuxUDPIPv4Socket() {
	if (fd >= 0) {
		destroy();
	}
}

void LinuxUDPIPv4Socket::initialize() {
...
void LinuxUDPIPv4Socket::destroy() {
	ASSERT(fd >= 0);
...
bool LinuxUDPIPv4Socket::sendPacketImpl(const NetworkAddress& netaddress, const void* data, unsigned int bytecount) {
...
int LinuxUDPIPv4Socket::receivePacketImpl(void* data, unsigned int bytecount, NetworkAddress* netaddress) {
	ASSERT((netaddress->getAddressType() & (NetworkAddressType::MASK_IP_VERSION)) == NetworkAddressType::IPV4);
