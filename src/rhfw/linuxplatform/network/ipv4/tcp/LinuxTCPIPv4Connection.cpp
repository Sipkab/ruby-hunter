...
LinuxTCPIPv4Connection::LinuxTCPIPv4Connection() {
}

LinuxTCPIPv4Connection::LinuxTCPIPv4Connection(const TCPIPv4Address& address, int fd)
...
int LinuxTCPIPv4Connection::readImpl(void* buffer, unsigned int count) {
	WARN(fd < 0);
	int res;
...
int LinuxTCPIPv4Connection::writeImpl(const void* data, unsigned int count) {
	WARN(fd < 0);
	int res;
...
bool LinuxTCPIPv4Connection::connectImpl(const NetworkAddress& netaddress) {
...
void LinuxTCPIPv4Connection::disconnect() {
	WARN(fd < 0);
	if (fd < 0) {
		return;
	}
