...
LinuxNetworkAdapter::LinuxNetworkAdapter() {
}
LinuxNetworkAdapter::~LinuxNetworkAdapter() {
}

LinkedList<LinuxNetworkAdapter> LinuxNetworkAdapter::getAdapters() {
	int res;
	LinkedList<LinuxNetworkAdapter> result;
	...
	}
	{
		...
		if (getifaddrs(&ifap) == 0) {
			for (ifaddrs * it = ifap; it != nullptr; it = it->ifa_next) {
				...
				LinuxNetworkAdapter* adapter = new LinuxNetworkAdapter();
