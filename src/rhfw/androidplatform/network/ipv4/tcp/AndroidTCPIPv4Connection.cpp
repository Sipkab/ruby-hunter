...
AndroidTCPIPv4Connection::AndroidTCPIPv4Connection() {
}

AndroidTCPIPv4Connection::AndroidTCPIPv4Connection(const TCPIPv4Address& address, int fd)
...
int AndroidTCPIPv4Connection::readImpl(void* buffer, unsigned int count) {
	WARN(fd < 0);
	int res;
...
int AndroidTCPIPv4Connection::writeImpl(const void* data, unsigned int count) {
	WARN(fd < 0);
	int res;
...
bool AndroidTCPIPv4Connection::connectImpl(const NetworkAddress& netaddress) {
...
void AndroidTCPIPv4Connection::disconnect() {
	WARN(fd < 0);
	if (fd < 0) {
		return;
	}
