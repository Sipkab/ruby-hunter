#ifndef RHFW_GEN_RESOURCES_H_
#define RHFW_GEN_RESOURCES_H_
...
namespace ResIds {
	static const unsigned int RESOURCE_COUNT = @this.resourcesmap.size()@;
	static const unsigned int QUALIFIED_RESOURCE_COUNT = 0;
@function exportminmax(list, tabstack):
	@opt Collections.sort(list)@
	@if list.size() == 0:
@String.join("", tabstack)@static __enumerator<ResId, 0x0, 0x0> enumerate() { return { }; }
	@else:
		@if list.get(list.size() - 1) - list.get(0) == list.size() - 1:

@String.join("", tabstack)@static __enumerator<ResId, 0x@Integer.toHexString(list.get(0))@, 0x@Integer.toHexString(list.get(list.size() - 1) + 1)@> enumerate() { return { }; }

		@else:

@String.join("", tabstack)@static __multi_enumerator<ResId
...
@String.join("", tabstack)@,__range_iterator<ResId, 0x@Integer.toHexString(start)@, 0x@Integer.toHexString(end+1)@>
...
@void tabstack.push("\t")@
@foreach item in this.resourcesmap.entrySet():
...
@String.join("", tabstack)@static const ResId @remain.replaceAll("[^0-9a-zA-Z_:]", "_")@ = (ResId) 0x@Integer.toHexString(item.getValue())@;
@
@call exportminmax(stack.pop(), tabstack)@
